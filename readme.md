...
- 테마는 단순 색상 반전이 아니라 **색상 심리, 시각 피로, 대비, 명도 일관성, 콘텐츠 인지 효율** 다섯 축과 실제 **조명 환경/사용 피로도/시각적 균형** 기준으로 설계되었습니다.
- 라이트·다크 팔레트, 토큰 체계, QA 체크리스트는 [`docs/theme-guidelines.md`](docs/theme-guidelines.md)에 정리되어 있습니다.
- 배포 전에는 다음을 확인하세요.
  1. 주요 화면(Assign/Members/Stats/Settings)을 라이트·다크 각각 캡처하고 Accessibility Insights로 대비비를 점검 (목표 4.5:1 이상, CTA 7:1 이상)
  2. StatsView 차트를 색각 이상 모드로 에뮬레이트하며 데이터 구분이 유지되는지 확인
  3. 30분 이상 사용하는 시나리오에서 눈부심/피로도를 기록하고 필요 시 `--color-surface-*` 간 명도 간격을 조정
