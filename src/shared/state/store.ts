...
import type { ActivityEntry, ActivityType, AppData, MembersEntry, PartAssignment, WeekData, Warning } from '@/shared/types'
import type { SlotDescriptor } from '@/shared/utils/assignment'
import { slotToLabel } from '@/shared/utils/assignment'
...
const normalizeSlot = (slot: SlotDescriptor): SlotDescriptor | null => {
	if (slot.role === '사이드') {
...
const getSlotValue = (draft: { part1: PartAssignment; part2: PartAssignment }, slot: SlotDescriptor): string => {
	if (slot.role === '사이드') {
...
const setSlotValue = (draft: { part1: PartAssignment; part2: PartAssignment }, slot: SlotDescriptor, value: string) => {
	if (slot.role === '사이드') {
		const idx = slot.index ?? 0
...
const slotsAreEqual = (a: SlotDescriptor, b: SlotDescriptor): boolean => {
