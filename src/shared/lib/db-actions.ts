...
export async function getAllData(): Promise<AppData> {
    ...
    const weeks = await sql`SELECT * FROM weeks ORDER BY week_date`;
    ...
    weeks.forEach(w => {
        // Neon returns week_date as a Date object or string depending on driver settings
        const dateStr = w.week_date instanceof Date ? w.week_date.toISOString().split('T')[0] : w.week_date;
